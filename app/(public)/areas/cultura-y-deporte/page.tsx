...
const CulturaPage = () => {
  ...
  useCachedFetch({
    selector: (state) => state.services.lastFetched,
    dataKey: 'resena.cultura',
    fetchAction: () => getAreaResenaAsync('cultura'),
    ttl: CACHE_TTL.SERVICES,
    hasData: !!resenas?.cultura,
  });
    ...
    <Box>
      ...
      <Container maxWidth="lg" sx={{ py: { xs: 6, md: 8 } }}>
        <AnimatedSection animation="fadeInUp">
          {services.length === 0 ? (
            <Alert severity="info">
              La información de servicios de cultura y deporte no está disponible en este momento.
            </Alert>
          ) : (
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, 1fr)' }, gap: 4 }}>
        ...
        <AreaGallery
          ...
          accentColor="#B52A1C"
          sectionTitle="Galería de Cultura y Deporte"
        />
      </Container>
    </Box>
  );
