...
const RegistroCivilPage = () => {
  const [galleryPhotos, setGalleryPhotos] = useState<GalleryPhoto[]>([]);

  useEffect(() => {
    fetch('/api/gallery/registro')
      .then((res) => res.json())
  ...
  useCachedFetch({
    ...
    dataKey: 'servicesByCategory.registro',
    fetchAction: () => getServicesByCategoryAsync('registro'),
    ttl: CACHE_TTL.SERVICES,
    hasData: servicesByCategory.registro.length > 0,
  });

  useCachedFetch({
    selector: (state) => state.services.lastFetched,
    dataKey: 'resena.registro',
    fetchAction: () => getAreaResenaAsync('registro'),
    ttl: CACHE_TTL.SERVICES,
    hasData: !!resenas?.registro,
  });

  const resena = resenas?.registro;

  const services = servicesByCategory.registro;
    ...
    <Box>
      ...
      <Container maxWidth="lg" sx={{ py: { xs: 6, md: 8 } }}>
        <AnimatedSection animation="fadeInUp">
          {services.length === 0 ? (
            <Alert severity="info">
              La información del Registro Civil no está disponible en este momento.
            </Alert>
          ) : (
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, 1fr)' }, gap: 4 }}>
        ...
        <AreaGallery
          ...
          accentColor="#2E7D32"
          sectionTitle="Galería del Registro Civil"
        />
      </Container>
    </Box>
  );
...
export default RegistroCivilPage;
